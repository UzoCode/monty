...
char *gets_int(int num);
unsigned int _abs(int);
int gets_numbase_len(unsigned int num, unsigned int base);
void fills_numbase_buff(unsigned int num, unsigned int base,
		       char *buff, int buff_size);

/**
 * gets_int - gets a character pointer to new string containing int
...
char *gets_int(int num)
{
	...
	temp = _abs(num);
	length = gets_numbase_len(temp, 10);
	...
	fills_numbase_buff(temp, 10, ret, length);
	if (num < 0 || num_l < 0)
		ret[0] = '-';
...
/**
 * gets_numbase_len - gets length of buffer needed for an unsigned int
...
/**
 * fills_numbase_buff - fills buffer with correct numbers up to base 36
